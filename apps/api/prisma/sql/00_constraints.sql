...
-- ------------------------------------------------ Colonne générée `balance`
-- Prisma crée `balance` en colonne simple ; on la convertit en colonne générée
-- STORED = amount_due - amount_paid (EF-PAY-02).
DO $$
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE parcels
      ADD COLUMN balance numeric(18,4)
